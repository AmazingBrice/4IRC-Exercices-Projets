...
int main(){
    int pid, df1, df2;
    char carac;
    df1 = open("./toto.txt", O_RDONLY);
    df2 = creat("./tata.txt", 0666);
    ...
    while (read(df1, &carac, sizeof(char)) > 0)
    {
        ...
        fflush(trace);
        (void) write(df2, &carac, sizeof(char));
    }
...
// Chaque processus fils travaille de concert les uns avec les autres en alternant les # et les $.